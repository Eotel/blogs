...
## Agent Harness vs User Harness（2層モデル）

2026年4月に整理された、より実践的な2層の区別:

| 層 | 定義 | 例 |
|---|---|---|
| **Agent Harness** | ツール作者が構築するインフラ層 | Claude Code 自体（CLAUDE.md 読み込み、フック実行、サブエージェント管理） |
| **User Harness** | ユーザーが設定するカスタマイズ層 | CLAUDE.md の記述・Skills・Hooks・MCP サーバー設定 |

User Harness は Agent Harness の上に乗るレイヤー。Claude Code の場合、ユーザーが制御できるのは User Harness のみ。

## ルールファイルを超えたハーネス設計（r.kagaya フレームワーク）

CLAUDE.md のようなルールファイルは User Harness の入口に過ぎない。r.kagaya 氏が提唱するより成熟したハーネス設計:

| 要素 | 内容 |
|------|------|
| **評価サイクル** | 出力品質を定量測定し、ハーネス自体を改善するフィードバックループ |
| **バージョン管理** | CLAUDE.md・Skills・Hooks を Git で管理し、変更履歴を追跡 |
| **CI/CD for Harness** | ハーネス変更を自動テストし、品質劣化を検知するパイプライン |
| **チーム共有** | APM（Agent Package Manager）で `apm.yml` に宣言・チーム全員が同一環境を再現 |

「ルールファイルに書き足す」段階から「ハーネス自体を製品としてエンジニアリングする」段階への移行が、AI ファースト開発の成熟度を示す。
...
- [Agent Harness vs User Harness — Claude Code の 2 層ハーネスを正確に理解する](/blogs/posts/2026/04/2026-04-23-harness-engineering-agent-vs-user-harness/) — 2026-04-23
- [ルールファイルを超えたハーネス設計 — 評価・バージョン管理・CI/CD で AI 出力品質を制御する](/blogs/posts/2026/04/2026-04-23-harness-engineering-beyond-rule-files/) — 2026-04-23
- [AWS が Skills フォーマットに合流 — Agent Toolkit for AWS の Skills を Strands Agents から呼ぶ](/blogs/posts/2026/05/2026-05-11-aws-agent-toolkit-strands-skills/) — 2026-05-11（Skills as interop layer の AWS 実装例）
