...
![TTS 推論高速化の 4 象限マップ。AR 投機的 TTS（VADUSA / Llasa+ / SSD）／ NAR Flow Matching 蒸留（F5-TTS / DMOSpeech 2）／ システム最適化（TensorRT-LLM / FP8 KV cache）／ ストリーミング設計（CosyVoice 2 / VoXtream）を代表手法と共に俯瞰した図。](/blogs/images/tts-acceleration-map.png)
...
![投機的 TTS のパイプライン図: 入力テキスト → Codec LM backbone → Target head と Draft heads × K に分岐 → Draft の候補ツリーを Target が 1 forward で並列検証 → tolerance sampling で accept、不一致は rollback、accept トークン列が Codec decoder で 24/48 kHz 波形に変換されるまでの流れ。](/blogs/images/speculative-tts-pipeline.png)
