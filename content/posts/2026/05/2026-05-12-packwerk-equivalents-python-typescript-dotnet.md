...
2. **可視性違反** (`enforce_privacy`): 各 pack の `app/public/` 配下のクラスだけを公開 API として扱い、**それ以外（`app/models/` などの通常ディレクトリを含む）への外部参照を禁止する**。`app/private/` という特別なディレクトリがあるわけではなく、`app/public/` 以外がすべて private とみなされる運用だ。

これらはいずれも、Ruby/Rails のソースを constant 参照レベルで静的解析して検出される。重要なのは、**Ruby に `internal` 修飾子も package private も存在しない**という前提だ。Go の `internal/` ディレクトリや Java の JPMS (`module-info`) のような **言語ネイティブの境界強制機構**がないからこそ、Packwerk のような「外付け」が必要になる。この前提を頭の片隅に置いておくと、他言語に行ったとき何が変わるかが見えやすい。
...
forbidden: [
  {
    ...
    to: {
      // 他 feature の internal/ への import を禁止。dependency-cruiser は
      // from.path のキャプチャグループを to 側で $1 として参照できる
      // 独自拡張を持つ（素の JS 正規表現の $1 とは別物で、ツールが
      // パス評価時に動的に展開する）。pathNot で同一 feature を除外する
      // ことで「他 feature への越境だけ」を残す。
...
- **Next.js (App Router)** には [`server-only`](https://www.npmjs.com/package/server-only) パッケージがあり、これを import するファイルがクライアントコンポーネントから読まれるとビルドが落ちる。**「サーバ/クライアントの境界」を build-time に強制する**点で、フレームワーク側が持っている数少ない「実効的な境界機構」と言える。一方で同じ Next.js の [private folders](https://nextjs.org/docs/app/getting-started/project-structure) (`_folder`) と route groups (`(group)`) は、**ルーティング対象から外す / レイアウト境界を切るためのファイル配置規約**であって、`_folder` 内のモジュールを他から import すること自体は止められない。import 境界として使うなら dependency-cruiser 等で別途ルールを書く必要がある。
...
| TypeScript | **dependency-cruiser** | 汎用 forbidden ルール | (Project References のみソフト) | Next.js の `server-only` が build-time 強制 |
...
| Java | ArchUnit / `module-info`（JPMS） | テスト + JPMS | named module 運用時のみ | Spring 系大型 |
...
1. **言語ネイティブの境界があれば、まず使い切る**。.NET の `internal` + csproj、Go の `internal/`、Java の JPMS `module-info`（named module + module-path 運用時に限る — 旧来の classpath 運用では強制されない）は、外付けツールより圧倒的に強い。CI 構成も増えない。
...
- フレームワーク（Hono / Next.js / NestJS / AdonisJS）の選択は、境界強制ツールの選択と直交する。Next.js の `server-only` のような **フレームワーク側で実効的に効くビルド時境界**だけは独自の強みなので併用する価値がある（`_folder` のようなルーティング規約は import 境界としては使えないので混同しない）。
