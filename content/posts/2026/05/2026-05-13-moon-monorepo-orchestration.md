...
```yaml
# .moon/toolchain.yml — 言語ランタイム / ツールチェイン
...
# project graph と action graph を Mermaid で出力
moon project-graph
...
moon の特徴で見過ごされがちなのが **task inheritance**。`.moon/tasks.yml` にグローバル task を定義し、各 `moon.yml` で部分的に上書きできる。

```yaml
# .moon/tasks.yml — 全プロジェクト共通
tasks:
  lint:
    command: 'eslint .'
  test:
...
|---|---|---|---|
| moon | Web ecosystem + 一部 polyglot、toolchain 同梱 | 中程度（Node / Bun / Deno / Rust / Go / Python / Ruby） | Bazel REAPI 互換セルフホスト |
| Turborepo (Vercel) | JS/TS 専用のシンプル task runner | 不可（[公式ドキュメント](https://turborepo.com/docs)上は JS/TS が前提） | Remote Cache (公式 / Vercel と統合) |
