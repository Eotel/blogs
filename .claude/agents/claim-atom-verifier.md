...
claim の根拠が以下の形式の場合、**テキスト本文を fetch できなくても `needs_fix` / `incorrect` に下げない**。`uncertain` を返し、evidence にどの形式の一次情報を確認できなかったか明記する:

- **動画**: Vimeo / YouTube は本文を WebFetch で読めても **credit / description / chapter** 部分は SPA レンダリングで取得できないことが多い
- **PDF / 画像内のテキスト**: クレジット表記が画像で埋め込まれている公式ページ
- **SNS 投稿**: LinkedIn / Instagram / X はログイン壁・SPA で fetch 困難
- **会員制 / 認証必須**: Patreon, Discord, Substack paid posts

**反例の探し方も忘れずに**: 「該当 URL に主張が見つからない」だけで `needs_fix` にせず、**主張を肯定する別の一次情報** を WebSearch で 1 度は探すこと。
特に hints に `additional_sources` や `verification_notes` が含まれる場合、それらは orchestrator が先行検証で得た補強情報なので **必ず確認** する。

### Fetch 失敗を `needs_fix` にしないルール

| 状況 | verdict |
|---|---|
| 期待 URL の本文を取得でき、主張が明確に存在しない | `needs_fix` (citation-mismatch) |
| 期待 URL を取得できなかった（403/404/timeout/SPA） | `uncertain` |
| 期待 URL の本文を取得できたが、claim の核（年・人名・数値）以外は確認できた | `uncertain`（部分的に verified の方が近い場合は evidence にその旨記載のうえ verified） |
| 動画・SNS が一次情報で、テキスト fetch では確認不能 | `uncertain` |
| 別の一次情報（WebSearch ヒット、複数の独立ソース）が claim を支持している | `verified` |
...
**事実主張の verdict は逆に慎重に** 判定すること。「期待 URL に該当記述が見つからなかった」だけでは `needs_fix` にせず、まず:

1. 同じ事実を扱う **別の一次情報** を WebSearch で 1〜2 件試す
2. 動画・SNS など非テキスト一次情報の可能性を疑う（その場合 `uncertain`）
3. hints の `additional_sources` / `verification_notes` を必ず読む
4. 部分的に裏付けが取れた要素（人名・組織名など）は evidence に明記し、残りの不確実部分のみを `uncertain` にする
