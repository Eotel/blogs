...
- 抽出キーワードは **shell 変数に代入して**から渡す（質問本文を直接コマンド引数に展開しない = command injection 防止）
- `grep -F`（fixed string）で正規表現解釈を避ける（`C++` `[abc]` `a+b` 等の誤マッチ防止）
- `-e "$KW"` または末尾 `--` でオプション解釈を遮断（`-x` 等で始まるキーワード対策）
- 引数内で `$()` を直接使わない（`AGENTS.md` の auto-mode ルール）。年などは別コマンドで変数に入れてから展開する

```bash
# キーワードを変数に入れる（質問本文を直接 grep に渡さない）
