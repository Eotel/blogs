...
- `.Codex/settings.local.json` の `permissions.allow` 配列
- AGENTS.md のルール
- `.Codex/skills/` 配下のスキル定義
...
- `/tmp` → `.Codex/temp/`（プロジェクト内の一時ディレクトリ）
- `&&` で繋いだコマンド → 個別実行
...
1. `.Codex/settings.local.json` — 許可パターンの追加
2. `.Codex/skills/*/SKILL.md` — コマンド例の修正
3. `AGENTS.md` — 必要に応じてルールの追記
