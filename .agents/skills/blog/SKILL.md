...
- **GitHub Issue コメント URL**: `https://github.com/Eotel/blogs/issues/{number}#issuecomment-{id}` 形式
...
1. **プロンプト本文に author 指定がある場合**: `blog-batch.sh` から呼び出された場合、プロンプトに `author id "<id>"` の指定が含まれる。その値を抽出して使用する
2. **GitHub Issue/コメント URL がソースの場合**: コメント投稿者の `github_login` を `scripts/authors.json` で `id` に解決する:
   ```bash
...
例:
```bash
# Write ツールで $WORKTREE_DIR/pr-url.txt に "https://github.com/Eotel/blogs/pull/141" を書き出す
bash .claude/scripts/update-issue-comment.sh Eotel blogs 4126127772 $WORKTREE_DIR/pr-url.txt
```
